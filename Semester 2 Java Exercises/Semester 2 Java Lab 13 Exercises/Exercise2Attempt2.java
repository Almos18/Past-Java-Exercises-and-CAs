//Exercise2Attempt2.java

import javax.swing.JOptionPane;
import java.util.Arrays;

public class Exercise2Attempt2 {
    public static void main(String args[])
    {
        String tNumbersArray[] = new String[10];
        String coursesArray[] = new String[10];
        String tNumbers, courses, temp, seek, largestCourse, largest;
        int sub, largestj, here;
        boolean found;
		...
		{
			  	largest = tNumbersArray[i];
			  	sub = i;
			  			
			  		for (int j = i+1; j < tNumbersArray.length; j++)
			       		if (tNumbersArray[j].compareTo(largest) < 0)
			       			{
				   				largest = tNumbersArray[j];
				   				sub = j;
			       			}
    ...
    public static void counterMethod(String tNumbersArray[], String coursesArray[], int courseCounter, int t003)
    {
        for(int h=0;h<coursesArray.length;h++)
            {
                if(tNumbersArray[h].startsWith("t003"))
                {              
                t003++;
                }              

                if(coursesArray[h].equals("Health and Leisure") || coursesArray[h].equals("Chemistry"))
                {
                courseCounter++;
                }
...
while(lowerSub<=higherSub)
{
    ...
    if(array[middleSub].compareTo(seek)<0)
    lowerSub=middleSub+1;
    else if(array[middleSub].compareTo(seek)>0)
    higherSub=middleSub-1;
    else
    return true;
}